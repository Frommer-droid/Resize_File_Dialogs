...
3. Для frozen-режима `VERSION` включается в сборку через `.spec`.

## 5. Сборка приложения

1. Запустить `Build_Tools/SpecCompiler.pyw`.
...
8. Для стадии installer/copy после сборки использовать отдельные скрипты:
   ...
   - если в новой сборке нет `settings.json` или `ahk_resizer_generator_settings.json`, `00_Move.pyw` сохраняет существующие portable-настройки;
